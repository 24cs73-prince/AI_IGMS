...
export default function Students() {
  ...
  const students = useMemo(() => {
    ...
    return rawStudents.map((s, idx) => {
      ...
      return {
        ...
        phone: String(
          s?.phone || "+91 98000 00000"
        ),

        email: String(
          s?.email || "student@igms.edu"
        ),

        admissionDate: String(
          s?.admissionDate || "2022-04-10"
        ),
      };
    });
  }, [rawStudents]);
  ...
  const getAuthToken = () => {
    try {
      ...
      if (rawUser) {
        return (
          JSON.parse(rawUser)?.token || ""
        );
      }
    } catch (error) {
      console.warn(
        "Could not read auth user:",
        error
      );
    }
  ...
  const handleCreateStudent = async () => {
    ...
    try {
      ...
      toast.success(
        `Student ${newStudent.name} saved successfully!`
      );
      ...
      toast.error(
        "Failed to add student. Please check the server."
      );
    }
  };
