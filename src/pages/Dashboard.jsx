...
export default function Dashboard() {
  ...
  const upcomingExams = exams.filter((e) => ['Upcoming', 'Scheduled'].includes(e.status)).slice(0, 4);
  const recentNotices = notices.slice(0, 4);
  const trendData = trend.map((t) => ({ month: t.month, value: t.performance }));
  const enrollmentData = enrollmentByClass.map((e) => ({ className: e.className, value: e.students }));

  return (
    <div>
      <PageHeader
        title="Dashboard"
        description="Welcome back — here's what's happening across your institution today."
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <ChartCard title="Today's Attendance" subtitle="Live breakdown by status">
          <div className="p-4">
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/examination" className="text-xs font-medium text-primary hover:text-primary-700">View all</Link>
          </div>
            ...
            {upcomingExams.map((e) => (
              <li key={e.id} className="flex items-center gap-3">
                <div className="flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-xl bg-primary/10 text-primary">
                  <span className="text-[10px] font-medium uppercase">{formatDate(e.date, { month: 'short', day: undefined, year: undefined })}</span>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-400">{e.className} · {e.time}</p>
                </div>
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/notices" className="text-xs font-medium text-primary hover:text-primary-700">View all</Link>
          </div>
          <ul className="space-y-3">
            {recentNotices.map((n) => (
              <li key={n.id} className="group">
                <Link to="/notices" className="block rounded-lg p-2 -mx-2 hover:bg-canvas transition-colors">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-sm font-medium text-ink line-clamp-1 group-hover:text-primary">{n.title}</p>
                    <FiArrowUpRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-slate-300 group-hover:text-primary" />
                  </div>
                  <p className="mt-0.5 text-xs text-slate-400">{n.author} · {formatDate(n.date)}</p>
                </Link>
              </li>
            ))}
          </ul>
        ...
        <Card>
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="flex items-center gap-1.5 text-xs font-medium text-accent">
              <span className="h-2 w-2 animate-pulse rounded-full bg-accent" /> Operational
            </span>
          </div>
          <ul className="space-y-3">
            {system.map((s) => (
              <li key={s.label} className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  <span className={`h-2 w-2 rounded-full ${s.tone === 'success' ? 'bg-accent' : s.tone === 'warning' ? 'bg-warning' : 'bg-danger'}`} />
                  <span className="text-sm text-slate-600">{s.label}</span>
                </div>
                <span className="text-xs font-medium text-slate-400">{s.uptime}</span>
              </li>
            ))}
          </ul>
        </Card>
