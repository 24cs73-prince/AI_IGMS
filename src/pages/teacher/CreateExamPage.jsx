...
export default function CreateExamPage() {
  ...
  const [formData, setFormData] = useState({
    title: "Class 1 English - Unit Test",
    classVal: "1",
    subject: "English",
    syllabus: "Alphabet, Phonics, Naming Words, Action Words, Animal Names",
    duration: "30 minutes",
    durationMinutes: 30,
    totalQuestions: 10,
    totalMarks: 10,
  ...
  const currentAvailableSubjects = SUBJECTS_BY_CLASS[formData.classVal] || SUBJECTS_BY_CLASS["5"];

  const handleClassChange = (opt) => {
    const available = SUBJECTS_BY_CLASS[opt.value] || SUBJECTS_BY_CLASS["5"];
    const firstSub = available[0].value;
    setFormData((prev) => ({
      ...prev,
      classVal: opt.value,
      subject: firstSub,
      title: `Class ${opt.value} ${firstSub} - Unit Test`,
    }));
  };
  ...
  const handleSubjectChange = (opt) => {
    setFormData((prev) => ({
      ...prev,
      subject: opt.value,
      title: `Class ${prev.classVal} ${opt.value} - Unit Test`,
    }));
  };
  ...
  const handleGenerateAndCreate = async (status = "Published") => {
    ...
    const messages = [
      ...
      `Reading syllabus for Class ${formData.classVal} ${formData.subject}...`,
      `Generating ${formData.totalQuestions} live curriculum-aligned questions...`,
      ...
      <AnimatePresence mode="wait">
        {!generating ? (
              ...
              <div className="space-y-6">
                <div>
                  <Input
                    ...
                    placeholder="e.g. Class 1 English - Unit Test"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Dropdown
                    ...
                    options={CLASSES}
                    value={CLASSES.find((c) => c.value === formData.classVal) || CLASSES[0]}
                    onChange={handleClassChange}
                  />

                  <Dropdown
                    label={`Subject for Class ${formData.classVal}`}
                    options={currentAvailableSubjects}
                    value={currentAvailableSubjects.find((s) => s.value === formData.subject) || currentAvailableSubjects[0]}
                ...
                <div>
                  ...
                  <textarea
                    rows={3}
                    placeholder="Enter chapters or specific topics, e.g. Alphabet, Phonics, Naming Words..."
                  ...
                  <span className="text-[11px] text-slate-400 mt-1 block">
                    ⚡ Note: Groq AI will generate questions strictly covering this syllabus.
                  </span>
                </div>
          ...
          >
            ...
            <h3 className="mt-8 text-xl font-bold text-ink">Groq AI Engine is Generating Exam Paper...</h3>
